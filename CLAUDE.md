...
- **System-prompt note** (`agentIdentityNote`, injected by `agentCommand`): lasso knows the id at spawn time, so it tells the agent its id (and `$LASSO_TAB_ID`) up front — guaranteed in context on turn 1. It rides the **system** prompt (claude's `--append-system-prompt`), never the user prompt, so it doesn't pollute the agent's visible turn-1 message. codex has no `--append-system-prompt` equivalent, so there it falls back to a fenced footer on the user prompt.
