...
// ?view=files). We use real query params rather than the hash so links are
// conventional and the fragment stays free. Updates use replaceState so they
// don't pile up history entries on every tab change.
...
// tab changes don't pile up history entries.
...
// params in one history operation — so a multi-param navigation is a single
// Back step, not one per key.
