...
func TestReconcileTabs(t *testing.T) {
	...
	good := t.TempDir()
	_ = insertWorkspace(Workspace{ID: "wg", Host: "local", Title: "g", WorkDir: good})
	...
	_ = insertWorkspace(Workspace{ID: "wd", Host: "local", Title: "d", WorkDir: gone})
...
func TestEnsureTabSessionRecreates(t *testing.T) {
	...
	dir := t.TempDir()
	_ = insertWorkspace(Workspace{ID: "w1", Host: "local", Title: "x", WorkDir: dir})
