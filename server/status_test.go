...
func TestPollOnceIgnoresDeadAgents(t *testing.T) {
	openTestDB(t)
	_ = appendAgent("local", AgentRecord{ID: "a1", Title: "T", Agent: "claude", WorkDir: "/x", CreatedAt: time.Now()})
	_ = insertWorkspace(Workspace{ID: "wa1", Host: "local", Title: "T", WorkDir: "/x"})
...
func TestTreeSignatureChanges(t *testing.T) {
	openTestDB(t)
	base := treeSignature()
	_ = insertWorkspace(Workspace{ID: "w1", Host: "local", Title: "A"})
